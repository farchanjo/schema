...
### Storage-schema migration v1 → v2 (ADR-0028)

The first `schema serve` (or `schema daemon`) after upgrading to a
binary that ships ADR-0028 detects an old `chunks_vec` virtual table
without the `kind` partition column and rebuilds it in place. The
migration logs:
...
- The scalar `chunks` table is preserved (it is the canonical row
  store). The vector table `chunks_vec` is dropped + recreated with
  the new declaration. Embeddings have to be regenerated by the
  next delta-sync.
- `meta.schema_version` advances from `1` to `2` so subsequent
  starts skip the rebuild.
- Latency at first start is dominated by the embed phase, not the
  migration itself: a 50 K-chunk corpus rebuilds the vec table in
  < 30 s but re-embedding takes whatever fastembed needs (CPU cap
  per ADR-0018; ≈ 2 minutes on M1).

To force a clean rebuild (skips the auto-migration path):

```bash
rm -rf ~/.cache/schema/projects/<project-name>-<hash>/
# or, preserving cache dir:
schema reset --config schema.toml --yes
```

### Embedding-recipe migration (ADR-0029)

Per-project flag `[embedding] query_passage_prefix` (or
`SCHEMA_EMBEDDING_QUERY_PASSAGE_PREFIX=true`) toggles the BAAI
bge-m3 dense-retrieval recipe. Every flip — false → true or true →
false — is a corpus-wide migration.

On project start, `align_embedding_recipe` reads
`meta.embedding_recipe`, compares it against the desired value
derived from the flag, and on mismatch:

1. Logs:
   ```text
   WARN embedding recipe mismatch (ADR-0029); resetting store, delta-sync will rebuild
   ```
2. Calls `reset_all` — wipes `chunks` (FTS5 + vec0 cascade) but
   keeps `meta`.
3. Stamps the desired recipe into `meta.embedding_recipe`.
4. Hands off to the next delta-sync, which walks the corpus from
   disk and re-embeds every passage under the new recipe.

Cost: equivalent to a full reindex. Order of seconds for small
corpora; on the alloy-specs corpus (~390 paths) ≈ 2 minutes
end-to-end on developer hardware.

Rollback is symmetric — flip the flag back, restart, and the
adapter rebuilds against the raw recipe.

To audit the current recipe without restarting:

```bash
sqlite3 ~/.cache/schema/projects/<project-name>-<hash>/store.db \
        "SELECT key, value FROM meta;"
# expected rows:
#   schema_version    | 2
#   embedding_recipe  | raw  (or bge-m3-query-passage)
```

### Score floor (ADR-0028)

`[retrieval] min_score = <0.0..1.0>` (or
`SCHEMA_RETRIEVAL_MIN_SCORE=...`) installs a cosine-similarity
floor on every nearest-neighbour query. Absent setting means raw
top-K (legacy behaviour). Recommended starting point for noisy
corpora: `0.30`. Tighten per project after observing weak hits.

The floor is purely a query-time filter — flipping it does not
trigger a rebuild and takes effect on the next process restart.
