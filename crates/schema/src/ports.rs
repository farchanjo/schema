...
// ─── Embedder ────────────────────────────────────────────────────────────
//
// Moved to `schema-core` (ADR-0034). The `Embedder` port, `EmbedError`,
// the BGE-M3 prefix constants, and the BGE_M3_DIMENSIONS constant are now
// shared between the schema and recall bounded contexts. Import from
// `schema_core::{Embedder, EmbedError, EMBEDDER_QUERY_PREFIX,
// EMBEDDER_PASSAGE_PREFIX}` directly — `crate::ports::Embedder` no longer
// exists. The crate's `pub_use` lint forbids re-exporting from here.
