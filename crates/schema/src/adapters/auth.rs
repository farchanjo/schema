...
//!
//! ADR-0027 reverted the multi-tenant validator shape ADR-0026 had introduced:
//! the shared daemon now uses **one** workstation bearer at a single `/mcp`
//! mount, and project scoping happens per tool call via the
//! `working_directory` parameter. Single-token `BearerValidator` is sufficient
//! again.
